...
def json_request(ver, date_now, login, password, sender_id, receiver_id, tariff, mode, goods, services):
    ...
    json_obj = json.dumps({'version': ver, 'dateExecute': date_now, 'authLogin': login, 'secure': password, 'senderCityId': sender_id, 'receiverCityId': receiver_id,
                            'tariffList': tariff, 'modeId': mode, 'goods': goods, 'services': services})
...
def get_price(city_id):
    ...
    response1 = json_request(VERSION, date_now, TEST_LOGIN, SECURE, SENDER_CITY_ID, city_id, TARIFFS1, MODE_ID, GOODS, SERVICES)
    response2 = json_request(VERSION, date_now, TEST_LOGIN, SECURE, SENDER_CITY_ID, city_id, TARIFFS2, MODE_ID, GOODS, SERVICES)
